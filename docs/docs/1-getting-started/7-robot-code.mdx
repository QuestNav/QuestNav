...
// Get the Quest pose
Pose2d questPose = questNav.getPoseFrame().questPose();
    ...
    <TabItem value="CTRE" label="CTRE Swerve Generator < v2025.3.1.0 ">
        ...
        **FOR ANY VERSION NEWER THAN v2025.3.1.0** PLEASE REFERENCE THE OTHER TAB
        ...
        if (questNav.isConnected() && questNav.isTracking()) {
            // Get the latest pose data from the Quest
            PoseFrame poseFrame = questNav.getPoseFrame();

            // Get the Quest pose
            Pose2d questPose = poseFrame.questPose();
