...
List<QuestNav.PoseFrame> poseFrames = questNav.getAllUnreadPoseFrames();

if (!poseFrames.isEmpty()) {
    // Get the most recent Quest pose
    Pose2d questPose = poseFrames.get(poseFrames.size() - 1).questPose();
    ...
    <TabItem value="CTRE" label="CTRE Swerve Generator < v2025.3.1.0 ">
        ...
        **FOR ANY VERSION NEWER THAN v2025.3.1.0** PLEASE REFERENCE THE OTHER TAB
        ...
        if (questNav.isTracking()) {
            // Get the latest pose data frames from the Quest
            var questFrames = questNav.getAllUnreadPoseFrames();

            // Loop over the pose data frames and send them to the pose estimator
            for (var questFrame : questFrames) {
                // Get the pose of the Quest
                var questPose = questFrame.questPose();
                // Get timestamp for when the data was sent
                var timestamp = questFrame.dataTimestamp();
