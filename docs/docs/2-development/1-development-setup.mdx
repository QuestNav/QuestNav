...
## Native Dependencies
QuestNav uses a number of native libraries. These library have been pre-compiled and checked into this repo. However, they need to be
...
- Visual Studio Community (or other edition, but not Visual Studio Code) with "Desktop development with C++" installed.
- The Android NDK. The easiest way to install this is to install Android Studio, and then install the SDK, including the NDK.

### Setup
Create the following envronment variables:
<dl>
  <dt>`NDK_HOME`</dt>
  <dd>Your NDK directory, such as `C:\Users\me\AppData\Local\Android\Sdk\ndk\30.0.16138531`</dd>

  <dt>`JAVA_HOME`</dt>
  <dd>Your Android Stuido JDK, such as `C:\Program Files\Android\Android Studio\jbr`</dd>
</dl>
